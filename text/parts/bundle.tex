...
Далее, заметим что для любого $x$, $s(x + \iu y) \le s(\iu y)$. \todo{How to prove? We could probably take a look at numerator and denominator in separate?}. После всех вышеупомянутых замечания, для $s(x + \iu y)$ можно дать довольно простую нижнюю оценку:
...
\end{aligned}
\end{equation*}
, то есть подынтегральное выражение симметрично относительно оси $y$, и, следовательно, достаточно доказать сходимость интеграла на отрезке6 $-\pi/2 \le t \le \pi/2$.
...
Заметим, что в качестве $Z_0$ можно взять любое число в полуинтервале $[C-R, Z)$ (оценка снизу не обязана быть непрерывной функцией), главное чтобы $f(y)$ была неотрицательно определенной. Для простоты вычислений нам подойдет $Z_0 = \frac{V}{4}$. \todo{plot?}. Далее, отдельно рассматриваем интегралы на участках $[C - R, \frac{V}{4})$ и $[\frac{V}{4}, Z)$.
...
Так как $s_1$ вогнута, строго возрастает, и для фиксированного $r$, нас интересуют только $y \le C + R$, мы можем оценить функцию линейной: % TODOPLOT
\[
s_4(y) = 
\frac{s_1(C + R)}{C + R - Z} (y - Z) % TODO actually, even C + R is okay but whatever
\]

\todo{Заметим что $s_1$ в точке $C + R$ ограничена константой бла бла бла}
...
Таким образом, $\int\limits_{Z}^{Z + \frac{1}{R}} \ln^2 f(y) dy = \frac{1}{R} ( \ln^2 (\frac{s_1(C + R)}{C + R - Z} \frac{1}{R}) - 2 \ln (\frac{s_1(C + R)}{C + R - Z} \frac{1}{R}) + 2)$. Так как $s_1(C+R)$ ограничена константами, не зависящами от $r$ \todo{показать, это важно в доказательствах для других систем}, раскрывая логарифмы, легко можно понять что выражение является $\mcO(\frac{\ln^2 R}{R})$. Учитывая что коэффициент перед интегралом \todo{ссылку} порядка $\frac{1}{\sqrt{R}}$ \todo{???}, очевидно что интеграл на данном интервале стремится к нулю при $R, C \to \infty$.

\subsubsection{$Z + \frac{1}{R} \le y < C$}
Оценим сверху подынтегральное выражение \todo{ссылку}:
\begin{align*}
       & \ln^2 f(y) \frac{R}{R^2 - C^2 + 2 (C + 1) y + 1} \frac{1}{\sqrt{R + C - y}} \frac{1}{\sqrt{R - C + Z}}
...
\todo{Essentially, we have to integrate blah-blah}. Первообразная для подобного интеграла широко известна:
\[
